...
{
    [Bot("A_Strategy_MACD_4_2")] // We create an attribute so that we don't write anything to the BotFactory
    public class A_Strategy_MACD_4_2 : BotPanel
    {
        private BotTabSimple _tab;

        // Basic Settings
        private StrategyParameterInt Regime;
        ...
        public StrategyParameterInt AtrFilterIsOn;
        ...
        public StrategyParameterInt VolatilityFilterIsOn;
        ...
        public A_Strategy_MACD_4_2(string name, StartProgram startProgram) : base(name, startProgram)
        {
            TabCreate(BotTabType.Simple);
            _tab = TabsSimple[0];

            // Indicator setting
            _EmaPeriod = CreateParameter("Ema Length", 50, 50, 200, 50, "Base");
            ...
            // Exit
            TrailingValue = CreateParameter("Stop Value", 1.0m, 1, 2, 0.5m, "Base");
            
            // Basic setting
            Regime = CreateParameter("Regime: 0-On, 1-OnlyLong, 2-OnlyShort", 1, 0, 2, 1, "Base");

            // ATR
            AtrFilterIsOn = CreateParameter("Atr filter: 0-Off, 1-On,", 1, 0, 1, 1, "Base");
            AtrLength = CreateParameter("Atr length", 26, 10, 80, 3, "Base");
            AtrGrowPercent = CreateParameter("Atr grow percent", 3, 1.0m, 5, 2, "Base");
            AtrGrowLookBack = CreateParameter("Atr grow look back", 10, 10, 30, 10, "Base");

            // Volatility Stages
            VolatilityFilterIsOn = CreateParameter("Volatility filter: 0-Off, 1-On", 1, 0, 1, 1, "Base");
            VolatilityStageToTrade = CreateParameter("Volatility stage to trade", "2", new[] { "1", "2", "3", "4" }, "Base");
            VolatilitySlowSmaLength = CreateParameter("Volatility slow sma length", 26, 10, 80, 3, "Base");
            VolatilityFastSmaLength = CreateParameter("Volatility fast sma length", 12, 10, 80, 3, "Base");
            VolatilityChannelDeviation = CreateParameter("Volatility channel deviation", 0.5m, 0.1m, 0.5m, 0.4m, "Base");
            ...
            _volatilityStages = IndicatorsFactory.CreateIndicatorByName("VolatilityStagesAW3", name + "VolatilityStages", false);
            ...
            ParametrsChangeByUser += A_Strategy_4_2_ParametrsChangeByUser; ;
        ...
        private void _tab_PositionOpeningSuccesEvent(Position position)
        {
            if (AlertsRegime.ValueString == "On")
            {
                ...
                SendTelegramChannelMessageAsync(message);
            }
        }
        ...
        private void _tab_PositionClosingSuccesEvent(Position position)
        {
            if (AlertsRegime.ValueString == "On")
            {
                ...
                string flag = position.ProfitOperationPunkt * position.MaxVolume * position.Lots - position.ComissionValue * (position.EntryPrice + position.ClosePrice) * position.MaxVolume * position.Lots / 100 > 0 ? "🟩🟩🟩" : "🟥🟥🟥";

                string message = flag + " %23" + position.Number + "_" + _tab.NameStrategy + " " + position.State +
                "\r\n" + "Позиция: " + positionDirection +
                "\r\n" + "Направление: " + positionState +
                "\r\n" + "Бумага: $" + position.SecurityName +
                "\r\n" + "Цена входа: " + position.EntryPrice +
        ...
        private void A_Strategy_4_2_ParametrsChangeByUser()
        {
            ((IndicatorParameterInt)_Ema.Parameters[0]).ValueInt = _EmaPeriod.ValueInt;
            _Ema.Save();
            _Ema.Reload();
        ...
        private void LogicOpenPosition(List<Candle> candles)
        {
            ...
            if (openPositions == null || openPositions.Count == 0)
            {
                    ...
                    && Regime.ValueInt != 2)
                {
                    ...
                    if (VolatilityFilterIsOn.ValueInt == 1)
                    {
        ...
        // Method sending message to Telegram Channel
        private async void SendTelegramChannelMessageAsync(string message)
        {
            // Collecting query string
            string reqStr = "https://api.telegram.org/bot1664476800:AAELcJaqm3myJzi2ER4DGw9pVdnf3lYL5PM/sendMessage?chat_id=-1002450626729&text=" + message;
